...
% function [inertial cov_smooth seg P P_smooth dx dx_smooth] = smoothed_ZUPTaidedINS(inertial)
function inertial = smoothed_ZUPTaidedINS(inertial)
...
% Run the detector.
inertial.zupt = detector(u);

global simdata;

% Length of data
N = length(inertial.zupt);
...
while (1)
    ...
    inertial.pos = x(1:3,:);
    for k = 1:size(x,2)
        inertial.R(:,:,k) = Rotation_matrix(x(7:9,k));
    end
