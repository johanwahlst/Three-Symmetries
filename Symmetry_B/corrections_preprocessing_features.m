function [inds,x_features,y_yaw,y_pos,inertial_euler] = corrections_preprocessing_features(inertial,ground_truth,inds,Euler_angles)
...
x_features = zeros(size(Euler_angles,2),3,length(inds)-1);
for l = 1:size(Euler_angles,2)
RM = Rotation_matrix(Euler_angles(:,l)); 
for k = 2:length(inds)
    x_features(l,:,k-1) = var((RM*inertial.u(4:6,inds(k-1):inds(k)))');
end
end
